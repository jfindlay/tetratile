...
from tetratile.tetratile import Config, Dim, Grid, Polyomino, Translation, Rotation, tetrominoes
...
class TestPolyomino:
    ...
    def test__check_grid(self):
        ...
        for i in range(random.randint(min(self.grid.width, self.grid.height), self.grid.width*self.grid.height)):
            self.grid[random.randint(0, self.grid.width - 1), random.randint(0, self.grid.height - 1)]['type']:str = random.choice(tetrominoes).name
        for v, square in self.grid:
